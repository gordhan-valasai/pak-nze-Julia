...
function add_objective!(model::Model, sets::ModelSets, params::ModelParameters)
    battery_capex = get(params.capex_usd_per_kw, "battery_storage_4h", 0.0)
    battery_fom = get(params.fixed_om_usd_per_kw_yr, "battery_storage_4h", 0.0)
    @objective(model, Min, 3.4 * sum(params.discount_factor[y] * (
        sum(params.capex_usd_per_kw[t]*1000*model[:new_capacity_gw][t,y] for t in sets.technologies) +
        sum(params.fixed_om_usd_per_kw_yr[t]*1000*model[:installed_capacity_gw][t,y] for t in sets.technologies) +
        ...
        sum(params.carbon_price_usd_per_tco2[y]*model[:emissions_mtco2][t,y]*1e-6 for t in sets.technologies) +
        sum(get(params.negative_emission_cost_usd_per_tco2, t, 0.0) * model[:negative_emissions_mtco2][t,y] * 1e-6 for t in sets.technologies) +
