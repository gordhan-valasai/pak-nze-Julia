...
function build_parameters(sets::ModelSets, scenario_config::AbstractDict, constraints_config::AbstractDict, technology_config::AbstractDict, selected_scenario::String)::ModelParameters
    ...
    capex_usd_per_kw = Dict{String,Float64}(); fixed_om_usd_per_kw_yr = Dict{String,Float64}(); var_om_usd_per_mwh = Dict{String,Float64}(); efficiency_fraction=Dict{String,Float64}(); capacity_factor=Dict{String,Float64}(); emission_factor_tco2_per_mwh=Dict{String,Float64}(); sector_by_technology=Dict{String,String}()
    for row in technology_config["technologies"]
        t=String(row["name"]); capex_usd_per_kw[t]=Float64(row["capex_usd_per_kw"]) ; fixed_om_usd_per_kw_yr[t]=Float64(row["fixed_om_usd_per_kw_yr"]) ; var_om_usd_per_mwh[t]=Float64(row["var_om_usd_per_mwh"]) ; efficiency_fraction[t]=Float64(row["efficiency_fraction"]) ; capacity_factor[t]=Float64(row["capacity_factor"]) ; emission_factor_tco2_per_mwh[t]=Float64(row["emission_factor_tco2_per_mwh"]) ; sector_by_technology[t]=String(get(row,"sector","power"))
    end
    capex_usd_per_kw["beccs_power"] = 4500.0; fixed_om_usd_per_kw_yr["beccs_power"] = 90.0; var_om_usd_per_mwh["beccs_power"] = 18.0; efficiency_fraction["beccs_power"] = 0.85; capacity_factor["beccs_power"] = 0.85; emission_factor_tco2_per_mwh["beccs_power"] = -1.1; sector_by_technology["beccs_power"] = "power"
    capex_usd_per_kw["dac"] = 1100.0; fixed_om_usd_per_kw_yr["dac"] = 50.0; var_om_usd_per_mwh["dac"] = 0.0; efficiency_fraction["dac"] = 1.0; capacity_factor["dac"] = 0.90; emission_factor_tco2_per_mwh["dac"] = -1.0; sector_by_technology["dac"] = "industry"
    ...
    max_potential_gw=Dict(t=>1e6 for t in sets.technologies); max_buildrate_gw_per_year=Dict(t=>1e6 for t in sets.technologies)
    for (t,vals) in constraints_config["build_limits"]
        max_potential_gw[String(t)] = Float64(vals["max_potential_gw"]); max_buildrate_gw_per_year[String(t)] = Float64(vals["max_buildrate_gw_per_year"])
    end
    max_potential_gw["beccs_power"] = 10.0
    max_potential_gw["dac"] = 12.0

    carbon=Float64(get(sc,"carbon_price_usd_per_tco2",0.0)); carbon_price_usd_per_tco2=Dict(y=>carbon for y in sets.years)
    ...
    for g in sets.greenhouse_gases, t in sets.technologies
        ...
        gas_emission_factor_mt_per_mwh[(g, t)] = g == "CO2" ? base : 0.0
    end
    gas_emission_factor_mt_per_mwh[("CH4","rlng_ccgt")] = 2.0e-5
    gas_emission_factor_mt_per_mwh[("N2O","thar_coal_domestic")] = 1.5e-6
